...
export default function TaskItem({ task, folderId, checklistId, folderColor }: TaskItemProps) {
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`group flex items-center gap-3 md:gap-4 px-3 md:px-4 py-2.5 md:py-2 rounded-md transition-all duration-150 ${
        isDragging ? 'z-50 shadow-[var(--shadow-sm)] bg-[var(--card-solid)]' : 'hover:bg-[var(--card-hover)]'
      }`}
    >
      {/* Drag handle */}
      <div
      ...
      {isEditingDueDate && (
        <input
          ...
          className="px-2 py-1 bg-[var(--background)] border border-[var(--border)] rounded-md text-[var(--foreground)] text-xs focus:outline-none focus:border-[var(--border-hover)]"
          autoFocus
        />
      )}
      ...
      {!task.dueDate && !isEditingDueDate && (
        <button
          onClick={() => setIsEditingDueDate(true)}
          className="p-1.5 rounded-lg opacity-0 group-hover:opacity-100 hover:bg-[var(--card-hover)] transition-all"
          title="Add due date"
        >
